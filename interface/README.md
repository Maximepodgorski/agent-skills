...
5. **Ships** the interface from the spec using a ship loop
6. **Reviews** the result against reuse compliance and interface principles
...
/interface spec "settings page"
  ...
  3 ASCII wireframe options with coverage %
     │
     ▼  (user picks)
  Full interface spec → ds/interfaces/active/
     │
     ▼  /interface ship
...
| **`spec`** | Every screen | Propose layouts → spec file | `ds/interfaces/active/` |
...
```bash
/interface spec "dashboard"    # Propose + spec
/interface ship                # Implement
```

For complex or critical screens, add the quality gates:

```bash
/interface spec "admin dashboard"
/interface spec-review          # 3-6 expert perspectives review the spec
/interface ship
/interface review               # Post-ship compliance check
```
...
When you run `/interface spec "settings page"`, the skill:
...
```bash
/interface spec figma.com/design/abc123/file?node-id=1-2
/interface spec "settings page" figma.com/design/abc123/...
```
...
```bash
/interface spec "workspace settings"
# → See 3 layout options (Sidebar+Tabs, Single Column Form, Master-Detail)
# → Pick "A" → Spec generated at ds/interfaces/active/2026-03-24-workspace-settings.md
...
```bash
/interface spec "analytics dashboard" figma.com/design/abc/file?node-id=10-5
